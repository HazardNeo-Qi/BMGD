...
# 一次读取多个batch，构成buffer
def read_a_buffer(st, ed):
    images, labels = load_list(c.train_list_path, c.train_data_path)
    _st = min(st, c.train_num-1)
    _ed = min(ed, c.train_num-1)
    #print(_st, _ed)
    return images[_st:_ed], labels[_st:_ed]

def train_iterator_buffer(images_buffer, labels_buffer):
    dataset = tf.data.Dataset.from_tensor_slices((images_buffer, labels_buffer))
    dataset = dataset.shuffle(len(images_buffer))
    dataset = dataset.repeat()
    dataset = dataset.map(lambda x, y: tf.py_function(load_image, inp=[x, y, True, False], Tout=[tf.float32, tf.float32]),
                          num_parallel_calls=tf.data.experimental.AUTOTUNE)
    dataset = dataset.batch(c.batch_size)
